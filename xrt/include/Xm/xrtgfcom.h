...
/** \file xrtgfcom.h
 * Definitions necessary for API and XRT implementation.
 * \version 4.5
 * \date 01/04/04
 * \author Sitraka Inc.
 *
...
#if defined(XRT_IMPL) && defined(NDEBUG)
static char	sccsid_h2[] = "@(#)xrtgfcom.h	4.5	01/04/04 Sitraka Inc.";
#endif

#define XRT_USE_FLOAT 1
...
#define XRT_RELEASE_STRING   "4.0.0"

#define XRT_DEFAULT_FLOAT               XRT_HUGE_VAL


#define XRT_MAX_PRECISION               6
...
#define XRT_DEFAULT_OTHER_LABEL         "Other"
...
typedef char *              XrtColor;      /**< colors specified as strings */
typedef Boolean             XrtBoolean;    /**< use native Motif type */
typedef unsigned short      XrtDimension;  /**< use native Motif type */
typedef short               XrtPosition;   /**< use native Motif type */
typedef XmFontList          XrtFont;


#define XRT_TYPES_DEFINED
#endif /* XRT_TYPES_DEFINED */

typedef XtPointer			XrtImage;



typedef void    *XrtTextHandle;         /**< handle to attached text */
typedef void    *XrtDataHandle;         /**< handle to an XRT data structure */
typedef void    *XrtPointStyleHandle;   /**< handle to point style */
...
typedef enum {
	...
	/* Functions */
	XRT_COLOR_FUNC                      = XRT_PROPERTY(FUNCTION, 0),
...
typedef enum {
    ...
    XRT_REASON_MODIFY_END,
    XRT_REASON_ROTATE,
    XRT_REASON_TRANSFORM,
    XRT_REASON_ZOOM_AXIS,
    XRT_REASON_MAP,
    XRT_REASON_PICK
} XrtReason;
...
typedef struct {
    ...
    float             hole;			/**< value to use as the HOLE_VALUE */
...
typedef struct {
    int              reason;        /**< read only */
    XEvent          *event;         /**< read only */
    String          *params;        /**< read only */
    Cardinal         nparams;       /**< read only */
...
typedef struct {
    ...
    float            new_yaxis_min;
    float            new_yaxis_max;
    float            new_y2axis_min;
    float            new_y2axis_max;
    XrtBoolean       doit;
...
typedef struct {
    ...
    unsigned long     resh;     /**< reserved */
    unsigned long     ress;     /**< reserved */
} XrtDataStyle;
